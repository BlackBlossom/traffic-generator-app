...
class TrafficAnalytics {
  ...
  /**
   * Get raw logs from Redis for a specific campaign and user
   * @param {string} campaignId - Campaign ID
   * @param {string} userEmail - User email
   * @returns {Array} Raw log entries
   */
  async getRawLogsFromRedis(campaignId, userEmail) {
    try {
      const logKey = `campaign:${campaignId}:user:${userEmail}:logs`;
      const rawLogs = await redisLogger.redis.lrange(logKey, 0, -1);
      
      return rawLogs.map(log => {
        try {
          return JSON.parse(log);
        } catch (e) {
          return { timestamp: new Date().toISOString(), level: 'error', message: log, sessionId: 'unknown' };
        }
      });
    } catch (error) {
  ...
  async getSessionData(limit = 50, userEmail, type = 'history') {
    try {
      ...
      for (const campaign of campaigns) {
        const rawLogs = await this.getRawLogsFromRedis(campaign._id.toString(), userEmail);
