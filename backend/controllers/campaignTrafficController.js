...
const { runTraffic } = require('../traffic-worker/traffic'); // Adjust path as needed
...
function logToWebsocket(ws, level, message, campaignId = null, userEmail = null) {
  ...
  // Persist to Redis if campaignId and userEmail are provided
...
async function campaignTrafficController(campaign, ws, userEmail) {
  ...
  logToWebsocket(ws, 'info', `Campaign ${campaignId}: Controller started.`, campaignId, userEmail);
  ...
  try {
    while (true) {
      ...
      await runTraffic(batchCampaign, ws, campaignId, userEmail); // Pass campaignId and userEmail for user-specific Redis logging
...
module.exports = { campaignTrafficController };
