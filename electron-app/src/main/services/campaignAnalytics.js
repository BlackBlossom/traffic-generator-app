...
const DashboardAnalytics = require('./dashboardAnalytics');
...
class CampaignAnalytics {
  ...
  async initializeCampaignAnalytics(campaignId) {
    try {
      ...
      await sqliteLogger.initializeCampaignLogs(campaignId);
  ...
  async getCampaignAnalytics(campaignId, includeHistory = true) {
    try {
      ...
      if (includeHistory) {
        analytics.logs = await sqliteLogger.fetchLogs(campaignId, null, 100);
        analytics.logCount = await sqliteLogger.getLogCount(campaignId);
      }
  ...
  async updateCampaignMetrics(campaignId, sessionData) {
    try {
      ...
      if (updatedCampaign) {
        ...
        await sqliteLogger.logCampaign(campaignId, message);
